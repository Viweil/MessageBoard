...
//删除产品分类节点--------------------------------------------------------------
{
   ...
   {	   
       window.self.location.href= Result+".php?Action=Del&ID="+ID;
   } 
}
//分类节点展开和折叠(备用)-------------------------------------------------------------
...
{
   var check; 
   if (!voidNum(document.AdminLogin.Name.value))
   {
   ...
   if (!voidNum(document.AdminLogin.Password.value))
   {
	  alert("请输入管理员密码。");
...
function UpSortPicture()  //更新刚修改的图片
{
...
function UpVerifyCode(){
  document.getElementById("PhotoSN").src = "../Include/VerifyCode.php?t="+Math.random();
}
// 点击显示颜色图码表--------------------------------------------------------------------		
